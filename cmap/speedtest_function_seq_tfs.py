...
def clqt_seq_speedtest_linear_tfs_seq(clqt, steps, t0, dt, sigma, mu, diffeq_solver):
    ...
    C_init_m = kappa * jnp.eye(mu.shape[0])
    ...
    A_seq, b_seq, C_seq = seqFwdBwdPass(
        clqt, steps, dt, t0, A_init_m, b_init_m, C_init_m, diffeq_solver, diffeq_solver
    )
    u_fwdbwd_seq, x_fwdbwd_seq = combine_seqFwdBwdPass(
        S_list_jax_seq,
        Kx_list_jax_seq,
        v_list_jax_seq,
        d_list_jax_seq,
